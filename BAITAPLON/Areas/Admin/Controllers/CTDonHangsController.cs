    ...
    public class CTDonHangsController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                iddh.ct.Clear();
                string sqlQuery = "Select SanPham.TenSanPham,SanPham.HinhAnh,CTDonHang.* from CTDonHang,SanPham where CTDonHang.IDSanPham=SanPham.ID and CTDonHang.IDDonHang=@iddh";
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        string tensp = (string)reader["TenSanPham"];
                        string anh = (string)reader["HinhAnh"];
                        Decimal dongia = (Decimal)reader["DonGia"];
                        ctdh ct = new ctdh { id = idsp, ten=tensp,img=anh, sl = slsp, gia = dongia };
                        iddh.ct.Add(ct);
                    }
                }
            }
